...
class Player:
        ...
        if len(self.path) > 0:
            ...
            def expand_half_vertically() -> None:
                ...
                while point_to_add != self.path[0]:
                    ...
                    if up_point[1] >= 0 and (
                            grid.points[up_point[0] // self.width][up_point[1] // self.height].state == TRAVERSABLE or
                            grid.points[up_point[0] // self.width][
                                up_point[1] // self.height].state == PATH) and up_point not in half_1:
                        point_to_add = up_point
                        continue
                    # Search downward
                    down_point = (point_to_add[0],
                                  point_to_add[1] + self.height)
                    if down_point[1] < grid.height * self.height and (grid.points[down_point[0] // self.width][
                                                                          down_point[
                                                                              1] // self.height].state == TRAVERSABLE or
                                                                      grid.points[down_point[0] // self.width][
                                                                          down_point[
                                                                              1] // self.height].state == PATH) and down_point not in half_1:
                        point_to_add = down_point
                        continue
                    # Search to the left
                    left_point = (point_to_add[0] -
                                  self.width, point_to_add[1])
                    if left_point[0] >= 0 and (grid.points[left_point[0] // self.width][
                                                   left_point[1] // self.height].state == TRAVERSABLE or
                                               grid.points[left_point[0] // self.width][left_point[
                                                                                            1] // self.height].state == PATH) and left_point not in half_1:
                        point_to_add = left_point
                        continue
                    # Search to the right
                    right_point = (
                        point_to_add[0] + self.width, point_to_add[1])
                    if right_point[0] < grid.width * self.width and (grid.points[right_point[0] // self.width][
                                                                         right_point[
                                                                             1] // self.height].state == TRAVERSABLE or
                                                                     grid.points[right_point[0] // self.width][
                                                                         right_point[
                                                                             1] // self.height].state == PATH) and right_point not in half_1:
                ...
                while point_to_add != self.path[0]:
                    ...
                    if down_point[1] < grid.height * self.height and (grid.points[down_point[0] // self.width][
                                                                      ...
                                                                      grid.points[down_point[0] // self.width][
                                                                          down_point[
                                                                              1] // self.height].state == PATH) and down_point not in half_2:
                        point_to_add = down_point
                        continue
                    # Search upward
                    up_point = (point_to_add[0], point_to_add[1] - self.height)
                    if up_point[1] >= 0 and (
                            grid.points[up_point[0] // self.width][up_point[1] // self.height].state == TRAVERSABLE or
                            grid.points[up_point[0] // self.width][
                                up_point[1] // self.height].state == PATH) and up_point not in half_2:
                        point_to_add = up_point
                        continue
                    # Search to the left
                    left_point = (point_to_add[0] -
                    ...
                    if left_point[0] >= 0 and (grid.points[left_point[0] // self.width][
                                                   left_point[1] // self.height].state == TRAVERSABLE or
                                               grid.points[left_point[0] // self.width][left_point[
                                                                                            1] // self.height].state == PATH) and left_point not in half_2:
                        point_to_add = left_point
                        continue
                    # Search to the right
                    right_point = (
                    ...
                    if right_point[0] < grid.width * self.width and (grid.points[right_point[0] // self.width][
                                                                     ...
                                                                     grid.points[right_point[0] // self.width][
                                                                         right_point[
                                                                             1] // self.height].state == PATH) and right_point not in half_2:
                        point_to_add = right_point
                        continue
            ...
            for _ in range(grid.width):
                for x in range(grid.width):
                    for y in range(grid.height):
                        if (x > 0 and grid.points[x - 1][y].state == CLAIMED and grid.points[x][
                            y].state == UNCLAIMED) or (
                                x < grid.width - 1 and grid.points[x + 1][y].state == CLAIMED and grid.points[x][
                            y].state == UNCLAIMED) or (
                                y > 0 and grid.points[x][y - 1].state == CLAIMED and grid.points[x][
                            y].state == UNCLAIMED) or (
                                y < grid.height - 1 and grid.points[x][y + 1].state == CLAIMED and grid.points[x][
                            y].state == UNCLAIMED):
    ...
    def move(self, grid: Grid) -> None:
        ...
        if self.velocity_x == 1 and (self.x // self.width) + 1 < grid.width and grid.points[(self.x // self.width) + 1][
            self.y // self.height].state == TRAVERSABLE:
            self.move_traversable(grid)
        elif self.velocity_x == -1 and (self.x // self.width) - 1 >= 0 and grid.points[(self.x // self.width) - 1][
            self.y // self.height].state == TRAVERSABLE:
            self.move_traversable(grid)

        if self.velocity_y == 1 and (self.y // self.height) + 1 < grid.height and grid.points[self.x // self.width][
            (self.y // self.height) + 1].state == TRAVERSABLE:
            self.move_traversable(grid)
        elif self.velocity_y == -1 and (self.y // self.height) - 1 >= 0 and grid.points[self.x // self.width][
            (self.y // self.height) - 1].state == TRAVERSABLE:
            self.move_traversable(grid)

        # Move player on unclaimed grid if player is pushing
        if self.is_pushing:
            if self.velocity_x == 1 and (self.x // self.width) + 1 < grid.width and \
                    grid.points[(self.x // self.width) + 1][self.y // self.height].state == UNCLAIMED:
                self.move_unclaimed(grid)
            elif self.velocity_x == -1 and (self.x // self.width) - 1 >= 0 and grid.points[(self.x // self.width) - 1][
                self.y // self.height].state == UNCLAIMED:
                self.move_unclaimed(grid)

            if self.velocity_y == 1 and (self.y // self.height) + 1 < grid.height and grid.points[self.x // self.width][
                (self.y // self.height) + 1].state == UNCLAIMED:
                self.move_unclaimed(grid)
            elif self.velocity_y == -1 and (self.y // self.height) - 1 >= 0 and grid.points[self.x // self.width][
                (self.y // self.height) - 1].state == UNCLAIMED:
                self.move_unclaimed(grid)
